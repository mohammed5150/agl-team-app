...
function App() {
  ...
  const addInviteEmployee = useCallback(async ({ email, name, section, designation, role, tier }) => {
    ...
    sendSlack(slackEvents.employeeInvited(newEmp.name, newEmp.email));
    return { ok: true, id };
  }, [employees]);
  ...
  const addInviteEmployeesBulk = useCallback(async (rows) => {
    ...
    if (created > 0) {
      sendSlack(slackEvents.bulkInvited(created));
    }
    return {
      total: rows.length,
      created,
  ...
  const finalizeOwnProfile = useCallback(patch => {
    ...
    setNav("dashboard");
    sendSlack(slackEvents.profileFinalized(currentUser.name));
  }, [currentUser]);
